...
function TrendingSection() {
  ...
  const filteredProducts = selectedCategoryId === "ALL" ? products : products.filter(
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-x-4 max-w-full overflow-x-auto pb-2">
          {categories?.map((category) => (
            <CategoryButton 
              key={category._id} 
              category={category}
              onClickProp={() => setselectedCategoryId(category._id)} // passing setselectedCategoryId(category._id) function as a prop to another component
